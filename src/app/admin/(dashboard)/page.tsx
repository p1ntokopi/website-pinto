...
} from "@/components/admin/dashboard/order-queue";
...
export default async function AdminDashboard() {
  ...
  const { start: todayStartISO, end: tomorrowStartISO } =
    jakartaDayBounds(today);
  const todayDisplay = new Date(`${today}T12:00:00+07:00`);
  ...
  const { data: realizedRevenue, error: realizedRevenueError } =
    await supabase.rpc("get_realized_revenue", {
      p_start: todayStartISO,
      p_end: tomorrowStartISO,
    });
  if (realizedRevenueError) {
    console.error("[dashboard realized revenue]", realizedRevenueError);
  }
  const revenueToday = Number(realizedRevenue ?? 0);
  // A silent zero is indistinguishable from a genuinely empty day, so the
  // KPI keeps a visible failure marker instead.
  const revenueUnavailable = Boolean(realizedRevenueError);
  ...
  const tablesData = (tables || []).map((table) => {
    ...
    const tableOrders = session
      ? sessionOrders.filter((o) => o.dining_session_id === session.id)
      : [];
    return {
      ...table,
      session: session
  ...
  const kpiStats = [
    {
      label: revenueUnavailable ? "Pendapatan (data tidak tersedia)" : "Pendapatan",
      value: revenueUnavailable ? "—" : formatIDR(revenueToday),
    },
    {
      label: "Meja Terisi",
      value: `${occupiedTables || 0}/${totalTables || 0}`,
    },
    ...
    <div className="mx-auto w-full max-w-[1240px] space-y-10">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 font-display text-3xl font-bold tracking-tight text-ink sm:text-4xl">
            Selamat {greetingForHour(jakartaHour)}, {firstName}.
          </h1>
      ...
      <div className="grid gap-10 lg:grid-cols-3 lg:gap-6">
        <section className="lg:col-span-2">
          ...
          <OrderQueue
            initialOrders={
              activeOrdersNormalized as unknown as OrderQueueOrder[]
            }
            role={role}
          />
        </section>
