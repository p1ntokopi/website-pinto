    ...
    >
      <div>
        <p
          className={cn(
            'text-sm',
            emphasis ? 'font-bold text-ink' : 'text-muted-text',
          )}
        >
          {label}
        </p>
...
export default async function OwnerFinancePage({
  ...
  const paidOrders = summary?.sales.paid_order_count ?? 0
        ...
        <>
              ...
              <div className="rounded-sm border border-border-custom bg-card px-4 py-2">
                ...
                {summary.sales.tax > 0 && (
                  <BreakdownRow
                    ...
                    value={formatIDR(summary.sales.tax)}
                  />
                )}
                ...
                <p className="pb-3 text-xs text-muted-text">
                  Mencakup pembayaran order lama dan tagihan sesi kasir; setara
                  dengan {formatNumberID(paidOrders)} order lunas pada periode
                  pembayaran ini.
                </p>
              </div>
            </section>
                  ...
                  <table className="w-full min-w-[420px] text-sm">
                    ...
                    <tbody className="divide-y divide-border-custom/60">
                      {summary.payment_breakdown.map((row) => {
                        const share =
                          netSales > 0
                            ? Math.round((row.total / netSales) * 100)
                            : 0
                        return (
                          <tr key={row.method}>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 divide-y divide-border-custom/60 rounded-sm border border-border-custom bg-card sm:grid-cols-5 sm:divide-x sm:divide-y-0">
              ...
              <SecondaryMetric
                label="Lunas"
                value={formatNumberID(summary.order_reconciliation.paid)}
                tone="positive"
              />
              <SecondaryMetric
                label="Belum Lunas"
                value={formatNumberID(summary.order_reconciliation.unpaid)}
                tone="negative"
              />
              <SecondaryMetric
                label="Menunggu Bayar"
                value={formatNumberID(
                  summary.order_reconciliation.pending_payment,
                )}
              />
              <SecondaryMetric
                label="Dibatalkan"
            ...
            <p className="text-xs text-muted-text">
              ...
              (− {formatIDR(summary.refund.total)}). Order yang dibatalkan tidak
              dihitung dalam pendapatan.
            </p>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 divide-y divide-border-custom/60 rounded-sm border border-border-custom bg-card sm:grid-cols-3 sm:divide-x sm:divide-y-0">
              <SecondaryMetric
                label="Kas Masuk"
                value={formatIDR(summary.cash_flow.in)}
                hint="Pembayaran tunai (CASH)"
              />
              <SecondaryMetric
                label="Kas Keluar"
                value={formatIDR(summary.cash_flow.out)}
                hint="Pengeluaran tunai"
              />
              <SecondaryMetric
            ...
            <p className="text-xs text-muted-text">
              Arus kas ≠ pendapatan: hanya transaksi tunai yang dihitung di
              sini; pembayaran via transfer/e-wallet masuk ke saldo terkait,
              bukan kas.
            </p>
          </section>
          ...
          <p className="text-xs leading-relaxed text-muted-text">
            ...
            terealisasi = setiap pembayaran berstatus PAID yang tanggal bayarnya
            jatuh pada periode ini, baik pembayaran per order maupun satu
            tagihan sesi kasir; setiap pembayaran dihitung satu kali. Order yang
            dibatalkan dikecualikan. Net Sales = Gross − Diskon − Refund +
            Penyesuaian.
          </p>
        </>
      ) : null}
