...
import { ChevronLeft, Receipt, Clock, User, FileText } from 'lucide-react'
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-display text-3xl font-bold tracking-tight">Pesanan {order.order_number}</h1>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-border/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="font-semibold text-lg">Perbarui Status</h3>
          <p className="text-sm text-muted-foreground">Geser pesanan ini melalui alur kerja operasional.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-border/50 overflow-hidden">
            <div className="p-6 border-b border-border/50 flex items-center justify-between">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <Receipt className="w-5 h-5 text-muted-foreground" /> Item
              </h2>
              <span className="font-semibold text-muted-foreground">{order.items?.length || 0} item</span>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border/50 space-y-6">
            <h2 className="font-bold text-lg flex items-center gap-2">
              <User className="w-5 h-5 text-muted-foreground" /> Info Pelanggan
            </h2>
            
            <div className="space-y-4">
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1">Nama</span>
                <span className="font-medium text-lg">{order.customer_name || 'Tamu'}</span>
              </div>
              
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1">Meja</span>
                {order.table ? (
              ...
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1">Catatan Pesanan</span>
                {order.notes ? (
              ...
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1">Dibuat Pada</span>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border/50">
            <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
              <Clock className="w-5 h-5 text-muted-foreground" /> Riwayat
            </h2>
                ...
                <div key={event.id} className="relative flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium">
                      Ditandai <span className="font-bold">{event.new_status}</span>
                    </p>
