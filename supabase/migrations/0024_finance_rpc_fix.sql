-- 0024_finance_rpc_fix.sql
-- Fixes get_financial_summary: the `beans` CTE computed avg_price
-- (revenue / units) directly in the same SELECT list as its aggregates, which
-- Postgres rejects ("column product_sales.revenue must appear in the GROUP BY
-- clause"). Aggregates are now computed in an inner subquery, then avg_price
-- is derived from the result. Everything else is unchanged from 0022.
...
begin
  ...
  beans as (
    select
      b.units,
      b.revenue,
      (b.revenue / nullif(b.units, 0))::numeric as avg_price,
      (select ps.name from product_sales ps where ps.product_type = 'COFFEE_BEAN' order by ps.units desc, ps.revenue desc limit 1) as top_product
    from (
      select
