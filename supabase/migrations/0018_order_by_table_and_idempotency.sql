-- 0018_order_by_table_and_idempotency.sql
-- Owner wants no customer name: orders are identified by table number instead.
-- Also adds idempotency so double-submission (double tap / network retry) never
...
-- The signature changed (p_customer_name -> p_request_id), so the old function
-- must be dropped before re-creating it.
