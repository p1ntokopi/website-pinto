-- 0034_finance_delete_and_clear.sql
-- 1. Updates get_financial_summary and get_realized_revenue to exclude soft-deleted orders (deleted_at is not null)
-- 2. Adds admin_clear_financial_data RPC for owner to reset/clear orders, expenses, and adjustments
...
begin
  ...
  with paid_candidates as (
    ...
    left join public.orders direct_o on direct_o.id = p.order_id
    where p.status = 'PAID'
      and (p.order_id is null or direct_o.deleted_at is null)
  ),
  ranked_paid as (
    select pc.*,
      row_number() over (
      ...
      and (
        ...
        or not exists (
          ...
          where target_order.id = rp.order_id
            and target_order.deleted_at is null
        )
      )
  ),
  canonical_paid as (
  ...
  payment_order as (
    ...
    join public.orders o on (
      (o.id = cp.order_id and o.deleted_at is null)
  ...
  orders_by_day as (
    ...
    where deleted_at is null
      and (created_at at time zone 'Asia/Jakarta')::date between p_start and p_end
    group by 1
  ),
  revenue_series as (
  ...
  order_reconciliation as (
    select count(*)::int as total,
      ...
      count(*) filter (where o.status <> 'CANCELLED' and exists (
        ...
        where paid_order.id = o.id
          and paid_order.deleted_at is null
      ))::int as paid,
    ...
    where o.deleted_at is null
      and (o.created_at at time zone 'Asia/Jakarta')::date between p_start and p_end
  ),
  cash_in as (
...
-- Owner-only mass reset/clear of financial records
create or replace function public.admin_clear_financial_data(
  p_scope text,
  p_start date default null,
  p_end date default null,
  p_reason text default 'Pembersihan data keuangan oleh Owner'
) returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
declare
  ...
  v_reason text := coalesce(nullif(btrim(p_reason), ''), 'Pembersihan data keuangan oleh Owner');
  v_orders_deleted int := 0;
  v_expenses_voided int := 0;
  v_adjustments_voided int := 0;
  v_sessions_closed int := 0;
begin
  select role into v_role from public.profiles
  where id = v_actor and is_active = true;
  if v_role is null or v_role <> 'owner' then
    raise exception using errcode = '42501', message = 'Hanya Owner yang dapat mereset data keuangan.';
  end if;
  ...
  if p_scope in ('ALL', 'ORDERS') then
    with updated_orders as (
      ...
      where deleted_at is null
        ...
        and (p_end is null or (created_at at time zone 'Asia/Jakarta')::date <= p_end)
      returning id
    )
    select count(*)::int into v_orders_deleted from updated_orders;
    ...
    select count(*)::int into v_sessions_closed from updated_sessions;
  end if;

  if p_scope in ('ALL', 'EXPENSES') then
    with updated_expenses as (
      ...
      set status = 'VOIDED',
          ...
          voided_by = v_actor
      where status = 'ACTIVE'
        and (p_start is null or expense_date >= p_start)
        and (p_end is null or expense_date <= p_end)
      returning id
    )
    select count(*)::int into v_expenses_voided from updated_expenses;
  end if;

  if p_scope in ('ALL', 'ADJUSTMENTS') then
    with updated_adj as (
      ...
      set status = 'VOIDED',
          ...
          voided_by = v_actor
      where status = 'ACTIVE'
        and (p_start is null or effective_date >= p_start)
        and (p_end is null or effective_date <= p_end)
      returning id
    )
    select count(*)::int into v_adjustments_voided from updated_adj;
  end if;

  insert into public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
  values (v_actor, 'finance.cleared', 'finance', gen_random_uuid(),
    jsonb_build_object(
      'scope', p_scope,
      'start', p_start,
      'end', p_end,
      ...
      'expenses_voided', v_expenses_voided,
      'adjustments_voided', v_adjustments_voided,
...
revoke all on function public.admin_clear_financial_data(text, date, date, text) from public, anon, authenticated;
grant execute on function public.admin_clear_financial_data(text, date, date, text) to authenticated;
